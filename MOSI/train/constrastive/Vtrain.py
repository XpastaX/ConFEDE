...
def Vtrain(exp_type=None, load_model=None, check=None, config=default_config):
    ...
    if check is None:
        ...
        check = check.copy()
    log_path = config.LOGPATH + "MOSI_VisionPretrain." + datetime.datetime.now().strftime(
        '%Y-%m-%d-%H%M') + '.log'
    metrics = Metrics()
    ...
    batch_size = config.MOSI.downStream.visionPretrain.batch_size
    ...
    num_warm_up = config.MOSI.downStream.visionPretrain.num_warm_up
...
def eval(model, metrics=None, eval_data=None, device=None, config=default_config):
    ...
    with torch.no_grad():
        ...
        for index, sample in enumerate(bar):
            ...
            truth.append(label)
            loss += _loss.item() * config.MOSI.downStream.visionPretrain.batch_size
...
def Vtest(check_list=None, config=default_config):
    ...
    log_path = config.LOGPATH + "MOSI_VisionPretrain_Test." + datetime.datetime.now().strftime(
        '%Y-%m-%d-%H%M') + '.log'
    model = VisionPretrain(config=config)
    device = config.DEVICE
    model.to(device)

...
def Ttrain(exp_type=None, load_model=None, check=None, config=default_config):
    ...
    lr = config.MOSI.downStream.textPretrain.lr
    total_epoch = config.MOSI.downStream.textPretrain.epoch
    decay = config.MOSI.downStream.textPretrain.decay
    num_warm_up = config.MOSI.downStream.textPretrain.num_warm_up

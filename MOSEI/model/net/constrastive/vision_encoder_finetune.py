...
class VisionEncoder(nn.Module):
    def __init__(self, name=None, fea_size=None, encoder_fea_dim=None, nhead=None, dim_feedforward=None,
        ...
        if nhead is None:
            nhead = config.MOSEI.downStream.vision_nhead
        if drop_out is None:
            drop_out = config.MOSEI.downStream.vision_drop_out
        if dim_feedforward is None:
            dim_feedforward = config.MOSEI.downStream.encoder_fea_dim
        if num_layers is None:
            num_layers = config.MOSEI.downStream.vision_tf_num_layers
...
class VisionPretrain(nn.Module):
    ...
    def save_model(self, name):
        # save all modules
        encoder_path = self.config.MOSEI.path.encoder_path + name + '_vision_encoder.ckpt'
        decoder_path = self.config.MOSEI.path.encoder_path + name + '_vision_decoder.ckpt'
